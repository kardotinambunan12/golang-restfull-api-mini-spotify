...
func TestGendreDelete(t *testing.T) {
	app := fiber.New()
	services.InitDatabaseForTest()

	app.Delete("/gendre/:id", GendreDelete)

	//positif case
	req, _ := http.NewRequest("DELETE", "/gendre/1", nil)
	...
	//negatife case
	req, _ = http.NewRequest("DELETE", "/gendre/500", nil)
	req.Header.Set("accept", "application/json")
	res, err = app.Test(req)
	utils.AssertEqual(t, nil, err, "send request")
